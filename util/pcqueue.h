...
#define UTIL_ABORT(type, string) std::cerr << string << std::endl; abort()
#define UTIL_ABORT_IF(cond, type, string) if (cond) { UTIL_ABORT(type, string); }
...
class Semaphore {
  public:
    explicit Semaphore(int value) : task_(mach_task_self()) {
      UTIL_ABORT_IF(KERN_SUCCESS != semaphore_create(task_, &back_, SYNC_POLICY_FIFO, value), ErrnoException, "Could not create semaphore");
    }
    ...
    void wait() {
      UTIL_ABORT_IF(KERN_SUCCESS != semaphore_wait(back_), Exception, "Wait for semaphore failed");
    }

    void post() {
      UTIL_ABORT_IF(KERN_SUCCESS != semaphore_signal(back_), Exception, "Could not post to semaphore");
    }
...
class Semaphore {
  public:
    explicit Semaphore(LONG value) : sem_(CreateSemaphoreA(NULL, value, 2147483647, NULL)) {
      UTIL_ABORT_IF(!sem_, Exception, "Could not CreateSemaphore " << GetLastError());
    }
    ...
    void wait() {
      switch (WaitForSingleObject(sem_, INFINITE)) {
        ...
        case WAIT_TIMEOUT:
          UTIL_ABORT(Exception, "Timeout on an infinite wait?!");
        case WAIT_FAILED:
    ...
    void post() {
      UTIL_ABORT_IF(!ReleaseSemaphore(sem_, 1, NULL), Exception, "Failed to release Semaphore " << GetLastError());
    }
...
class Semaphore {
  public:
    explicit Semaphore(unsigned int value) {
      UTIL_ABORT_IF(sem_init(&sem_, 0, value), ErrnoException, "Could not create semaphore");
    }
    ...
    void wait() {
      while (-1 == sem_wait(&sem_)) {
        UTIL_ABORT_IF(errno != EINTR, ErrnoException, "Wait for semaphore failed");
      }
    }

    void post() {
      UTIL_ABORT_IF(-1 == sem_post(&sem_), ErrnoException, "Could not post to semaphore");
    }
